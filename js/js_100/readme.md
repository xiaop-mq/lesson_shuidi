...
- 数组可以直接`根据索引获取对应的元素`，所以不管取哪个位置的元素`时间复杂度都是O(1)`
消耗时间几乎一致，差异可以忽略不计。
...
## let、const、var的实现原理
### 实现原理
- javascript中的函数运行在它们被定义的作用域里，而不是它们执行的作用域里。
js解析的时候，优先解析const，因为它不能修改的是栈内存在的值和地址。然后解析let因为没有块作用域可能底层有处理，最后解析var.
- const实现原理：
const声明一个只读的常量。一旦声明，常量的值就不可再改变。

- let应用场景：
for循环的计数器。

三者的区别：
- let、const声明的变量不会挂载到window上，而var声明的变量会。
- var声明存在变量提升，let、const不存在变量提升。
1. 变量提升：变量在声明之前使用，值为undefined
- let、const声明形成块作用域，而var不会。
- 同一作用域下let、const不能重复声明变量，而var可以。
- let、const参在暂时性死区

- const
1. 一旦声明就必须赋值，不能使用null占位。
2. 声明后值不可以修改
3. 如果声明的是复合类型数据，可以修改其属性

## react-router里的<link>标签和<a>标签有什么区别
- 最终渲染的DOM来看，这两者都是链接，都是<a>标签，区别是
1. <link>是react-router里实现路由跳转的链接，一般配合<route>使用。<link>的跳转行为指挥只会触发象匹配的<route>对应的页面内容更新，而不会刷新整个页面。
2. <a>标签普通的超链接，用于从当前页面跳转到href指向的另一个页面。
