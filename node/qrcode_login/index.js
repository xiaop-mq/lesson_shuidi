...
//登录的本质，   
// cookie uid  http 在头部都会带上 cookie不能太长 否则更耗时
...
function generateToken(data, secret) { 
  // 有效性  7200 
  let iat = Math.floor(Date.now() / 1000);   // token生成时间  以秒数生成
  let exp = Math.floor(Date.now() / 1000) + 60*60*24*15; // 有效期15天
  // jsonwebtoken  将一个json对象,变成一个web登录的token 
