...
1.React和vue的区别
相同点：
1）都是数据驱动页面 都尝试了响应式组件
2）都有DOM virtural组件方法  都使用props参数传递父子组件之前的数据
3）数据流动单向 都支持服务器的渲染
4）都有native方法 react有reactnative vue有vuex
不同点：
1）vue双向数据绑定 react单项数据绑定
2）在庞大的项目之中react是更快的
3）react适合规模庞大的项目 而vue适合小快的项目
4）使用风格: react使用jsx+inline style将html css文件放进js文件夹下
   Vue 使用了webpack+stylus-loader单页应用将css html js放在同一个文件夹里
...
7.1rem,2rem,1rpx,1px,之间的换算
1)rpx不论哪个型号的手机。屏幕宽度都是750rpx
   rpx和px转换 根据设计稿转换
Eg: 设计稿750px  1px=1rpx
设计稿640px   1px = 750/640rpx
2)rem:根据设计稿换算：
Eg: 设计稿750px 那么1rem = (750/20)rpx=37.5px
设计稿640px 那么1rem = (640/20)rpx = 32px

8.浏览器之间怎么进行通信，页面之间怎么进行通信
   调用localstorage cookies


9.localStorage、sessionStorage、cookie、session的区别
1)存储大小
   Cookie一般不超过4K
   SessionStorage:5M或更大
   Localstorage:5M或更大
2）数据有效期
   Cookie:服务器生成，可以设置失效时间
   Sessonstorage:浏览器关闭前有效
   Localstorage:永久有效
3）作用域
     Cookie:在同源窗口是共享的
     SessionStorage:同一浏览器窗口共享
     Localstorage: 所有窗口共享
4）通信
  Cookie:在浏览器和服务器之间来回传递
     sessionStorage: 仅在客户端保存，不参和服务器通信‘不会把数据发给服务器，仅在本地保存
     LocalStorage: 仅在客户端保存，不参和服务器通信‘不会把数据发给服务器，仅在本地保存
10.怎么进行水平垂直居中所有方法
1）text-align + line-height  文本对齐+行高
2）Text-align + vertical-align  文本对齐 + 垂直对齐
...
11.Redux原理底层以及怎么使用
 jianshu.com/p/1519690fce26





1.熟练的掌握html css js制作网页前端
2.熟悉jquery 了解react
3.小程序 web前端
4.es6特性
...
React面试考点
1.什么是声明式编程？
   声明式编程是一种编程范式，它关注的是你做什么，而不是如何做。它表达逻辑而不显式的定义步骤，意味着我们需要根据逻辑来计算要显示的组件，声明式编程的例子有HTML,SQL等。
2.声明式编程和命令式编程
  声明式编程：做什么。 命令式编程：如何做。
3.什么是函数式编程？
   函数式编程时声明式编程的一部分。
   函数式编程有些核心概念：
   1）不可变性:函数式编程中，你不能更改数据。复制数据副本更改。
   2）纯函数：始终接受一个或多个参数并计算参数并返回数据或函数的函数，没有副作用。
3）数据转换 
4）高阶函数：将函数作为参数或返回函数的函数:array.map() array.filter() array.reduce()
5）递归: 函数在满足一定条件之前调用自身的技术
6）组合：将所有较小的函数组合成更大的函数，最终，得到一个应用程序
4. 什么是react?
   React是简单的javascript UI库，用于构建高效，快速的用户界面。它是一个轻量级库，因此很受欢迎。遵循组件设计模式、声明式编程和函数式编程概念。使用虚拟DOM来有效的操作DOM。遵循高阶组件到低阶组件的单向数据流。
4.什么是virtual DOM及其工作原理？
   浏览器遵循HTML指令来构造文档对象模型。
工作原理: 真实DOM的javascript 对象表示。
7.什么是JSX？
  jsx是js的语法拓展，就像是一个拥有js全部功能的模板语言。它生成react元素，这些元素将在DOM中呈现。在JSX中我们结合了js和html ，并生成可以在DOM中呈现的react元素。
Html 我们使用表达式：{}
Js 我们使用表达式: <>
8.组件和不同类型
   React中一切都是组件，组件通常是一个js函数。
React不同类型的组件
1）函数/无状态/展示组件
函数或无状态组件是一个纯函数，它可以接收参数，并返回react 元素。这是没有任何副作用的纯函数。这些组件没有状态或生命周期方法。
2）类/有状态组件
有状态组件和生命周期方法可以通过setState()方法改变组件的状态。类组件是通过react创建的。他在构造函数中初始化，也可能是子组件。
3）受控组件
   在react 中处理输入表单的一种技术。
4）非受控组件
   有一种称为非受控组件的方法可以通过使用Ref来处理表单数据。在非受控组件中，Ref用于直接从DOM访问表单值，而不是事件处理程序
5）容器组件
   处理获取数据、订阅redux存储等组件。
6）高阶组件
   将组件作为参数生成另一个组件的组件
9.Props和state
1)props:只读属性，传递给组件以呈现UI和状态，我们可随时更改组件的输出。
10.Propstypes
   提供类型检查，并未其他开发人员提供很好的文档
11.如何更新状态以及如何不更新？
   React 使用this.setState()时合并状态，新的props和状态是异步的
12.组件生命周期方法
...
这个生命周期方法在ErrorBoundary类中使用
10）componentDidCatch()
这个生命周期方法在ErrorBoundary类中使用
13.超越继承央视的组合
14.在react 应用样式
1）外部样式表：导入外部样式，使用className而不是class
2）内联样式：props传递给HTML元素，属性为style
3）定义样式对象并使用它：因为我们将js对象传递给style属性，所以我们可以在组件定义style对象并使用它。
15. 什么是redux 及其工作原理？
   redux是react的状态管理库，它基于flux.redux简化了react的单向数据流，redux将状态管理完全从react中抽象出来。
  工作原理：它是如何工作的
...
mapStateToProps
mapDispatchToProps
12.什么是react-router-dom及其工作原理
React-dom-router是应用程序路由的库，react库没有路由功能，需要单独安装。
React-router-dom 提供两个路由器BrowserRouter和HashRouter。前者基于url的pathname段，后者基于hash段。
前者：http://127.0.0.1:3000/article/num1 
后者：http://127.0.0.1:3000/#/article/num1（不一定是这样，但#是少不了的）
13.什么是错误边界？
在react中，我们通常有个组件树，如果一个组件发生错误，它将破坏整个组件树
错误边界有两个作用：
1）如果发生错误，显示择退回UI
2）记录错误
14.什么是Fragments?
   使用fragments 我们不需要在DOM中添加额外的节点。我们只需用React.Fragments或者简写<>来包裹内容。
15.什么是传送门？
Portal 提供了一种将子节点渲染到存在于父组件以外的 DOM 节点的优秀的方案
16.什么是上下文？
  上下文是一种传递props的方法，而不用在每一层传递组件树
17.什么是Hooks？
   Hooks让我们在函数组件可以使用state和其他的功能
  Hooks基本规则：
   hooks应在外层使用，不应在循环嵌套和条件函数中使用
hooks应该只在函数组件中使用
18.如何提高性能？
1）适当的使用shouldcomponentupdate生命周期方法。
2）使用create-react-app来构建项目
3）不可变性是提高性能的关键
4）在显示列表或表格时始终使用keys
5）代码分离，只加载模块或部分所需的文件的技术
19.如何在重新加载页面时保留数据？
1）使用浏览器localstorage来保存应用程序的状态
20.如何在react 进行api调用？
  使用redux-thunk.redux-thunk是一个中间件。一旦它被引入到项目中，每次派发一个action时，都会通过thunk传递
...
2）模块化(Module)：ES5不支持原生的模块化，在ES6中模块作为重要的组成部分被添加进来。模块的功能主要由 export 和 import 组成
3）箭头（Arrow）函数：=>不只是关键字function的简写，它还带来了其它好处。箭头函数与包围它的代码共享同一个this,能帮你很好的解决this的指向问题。有经验的JavaScript开发者都熟悉诸如var self = this;或var that = this这种引用外围this的模式。但借助=>，就不需要这种模式了。
4）函数参数默认值
5）模板字符串：ES6支持模板字符串，使得字符串的拼接更加的简洁、直观。${}
6）解构赋值：可以方便的从数组或者对象中快速提取值赋给定义的变量。
7）延展操作符(Spread operator)：延展操作符...可以在函数调用/数组构造时, 将数组表达式或者string在语法层面展开；还可以在构造对象时, 将对象表达式按key-value的方式展开。
8）对象属性简写：在ES6中允许我们在设置一个对象的属性的时候不指定属性名。
9）Promise：Promise 是异步编程的一种解决方案，比传统的解决方案callback更加的优雅
10）支持let与const：在之前JS是没有块级作用域的，const与let填补了这方便的空白，const与let都是块级作用域。
